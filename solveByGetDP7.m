function solveByGetDP7(P,current,sigma,uniTag)
...
numOfTissue = 7; % hard coded across ROAST.
...
fprintf(fid,'%s\n',['DomainC = Region[{white, gray, csf, bone, skin, air,lesion, ' gelStr elecStr(1:end-2) '}];']);
fprintf(fid,'%s\n\n',['AllDomain = Region[{white, gray, csf, bone, skin, air, lesion,' gelStr elecStr usedElecStr(1:end-2) '}];']);
fprintf(fid,'%s\n\n','}');
...
fprintf(fid,'%s\n',['sigma[lesion] = ' num2str(sigma.lesion) ';']);
