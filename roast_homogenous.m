function roast_homogenous(subj,recipe,varargin)
% roast(subj,recipe,varargin)
%

% ROAST modification by Ainslie Johnstone 30.03.20 
% Makes current flow estimations for a homogenous mask of the scan
% The only modification is really the segmnetation, and also the default
% conductance.
%
% N.B. If you are not scanning a human, then you need to edit the eTPM
% file and add it as a 'template' in the same way as you would with other
...
while indArg <= length(varargin)
    switch lower(varargin{indArg})
        ...
        case 'conductivities'
            ...
            indArg = indArg+2;
         case 'template'
            template= varargin{indArg+1};
            indArg = indArg+2;
        otherwise
            error('Supported options are: ''template'', ''elecType'', ''elecSize'', ''elecOri'', ''T2'', ''meshOptions'', ''conductivities'', ''simulationTag'', ''resampling'', and ''zeroPadding''.');
    end
end
...
f ~exist('template','var')
    template = [];
else
    if ~exist(template,'file'), error(['The template MRI you provided ' template ' does not exist.']); end
    
    templateData = load_untouch_nii(template);
    if templateData.hdr.hist.qoffset_x == 0 && templateData.hdr.hist.srow_x(4)==0
...
if ~exist('conductivities','var')
    conductivities = struct('inside',1,'outside',2.5e-14,'gel',0.3,'electrode',5.9e7); % literature values
else
    ...
    if isempty(conductivitiesNam) || ~all(ismember(conductivitiesNam,{'inside'; 'outside';'gel';'electrode'}))
    ...
    if ~isfield(conductivities,'inside')
        conductivities.white = 1;
    else
        if ~isnumeric(conductivities.inside) || any(conductivities.inside(:)<=0)
            error('Please enter a positive number for the inside conductivity.');
        end
        if length(conductivities.inside(:))>1, error('Tensor conductivity not supported by ROAST. Please enter a scalar value for conductivity.'); end
    end
    if ~isfield(conductivities,'outside')
        conductivities.outside = 2.5e-14;
    else
        if ~isnumeric(conductivities.outside) || any(conductivities.outside(:)<=0)
            error('Please enter a positive number for the air conductivity.');
        end
        if length(conductivities.outside(:))>1, error('Tensor conductivity not supported by ROAST. Please enter a scalar value for conductivity.'); end
    end
...
if ~strcmp(baseFilename,'nyhead')
    ...
    if (isempty(T2) && ~exist([dirname filesep 'c1' baseFilenameRSPD '_T1orT2.nii'],'file')) ||...
        ...
        disp('======================================================')
      %  start_seg_homogenous(subjRSPD,T2, template);
    else
        disp('======================================================')
    ...
    if (isempty(T2) && ~exist([dirname filesep baseFilenameRSPD '_T1orT2_masks.nii'],'file')) ||...
        ...
        disp('======================================================')
      %  segTouchup_homogenous(subjRSPD,T2);
    else
        disp('======================================================')
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_mask_elec.nii'],'file')
    ...
    disp('======================================================')
    hdrInfo = electrodePlacement_homogenous(subj,subjRSPD,T2,elecName,options,uniqueTag);
else
    disp('======================================================')
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_v.pos'],'file')
    ...
    disp('======================================================')
    prepareForGetDP_homogenous(subj,node,elem,hdrInfo,elecName,uniqueTag);
    solveByGetDP_homogenous(subj,injectCurrent,conductivities,uniqueTag);
else
    disp('======================================================')
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_result.mat'],'file')
    ...
    visualizeRes_homogenous(subj,subjRSPD,T2,node,elem,face,vol_all,ef_mag,ef_all,injectCurrent,hdrInfo,uniqueTag,0);
else
    disp('======================================================')
    ...
    visualizeRes_homogenous(subj,subjRSPD,T2,node,elem,face,vol_all,ef_mag,ef_all,injectCurrent,hdrInfo,uniqueTag,1);
end