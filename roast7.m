function fin=roast7(subj,recipe,varargin)
...
if ~exist('conductivities','var')
    conductivities = struct('white',0.126,'gray',0.276,'csf',1.65,'bone',0.01,...
                       ...
                       'lesion',1); % literature values
else
    if ~isstruct(conductivities), error('Unrecognized format of conductivity values. Please enter as a structure, with field names as ''white'', ''gray'', ''csf'', ''bone'', ''skin'', ''air'', ''lesion'', ''gel'' and ''electrode''.'); end
    conductivitiesNam = fieldnames(conductivities);
    if isempty(conductivitiesNam) || ~all(ismember(conductivitiesNam,{'white';'gray';'csf';'bone';'skin';'air';'gel';'electrode';'lesion'}))
        error('Unrecognized tissue names detected. Supported tissue names in the conductivity option are ''white'', ''gray'', ''csf'', ''bone'', ''skin'', ''air'', ''lesion'',''gel'' and ''electrode''.');
    end
    ...
    if ~isfield(conductivities,'lesion')
        conductivities.lesion = 1;
    else
        if ~isnumeric(conductivities.lesion) || any(conductivities.lesion(:)<=0)
            error('Please enter a positive number for the lesion conductivity.');
        end
        if length(conductivities.lesion(:))>1, error('Tensor conductivity not supported by ROAST. Please enter a scalar value for conductivity.'); end
    end
...
Sopt = dir([dirname filesep baseFilename '_' simTag '_options.mat']);
if isempty(Sopt)
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_mask_elec.nii'],'file')
    ...
    disp('======================================================')
%     load([dirname filesep baseFilename '_' simulationTag '_labelVol.mat'],'volume_elecLabel','volume_gelLabel');
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '.mat'],'file')
    ...
    disp('======================================================')
    [node,elem,face] = meshByIso2mesh7(subj,subjRSPD,T2,meshOpt,hdrInfo,uniqueTag);
else
    disp('======================================================')
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_v.pos'],'file')
    ...
    disp('======================================================')
    prepareForGetDP7(subj,node,elem,hdrInfo,elecName,uniqueTag);
    solveByGetDP7(subj,injectCurrent,conductivities,uniqueTag);
else
    disp('======================================================')
    disp('           MODEL ALREADY SOLVED, SKIP STEP 5          ')
    disp('======================================================')
%     load([dirname filesep baseFilename '_' simulationTag '_elecMeshLabels.mat'],'label_elec');
end
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_result.mat'],'file')
    ...
    visualizeRes7(subj,subjRSPD,T2,node,elem,face,vol_all,ef_mag,ef_all,injectCurrent,hdrInfo,uniqueTag,0);
else
    disp('======================================================')
    ...
    visualizeRes7(subj,subjRSPD,T2,node,elem,face,vol_all,ef_mag,ef_all,injectCurrent,hdrInfo,uniqueTag,1);
end
fin='ROAST 7 is DONE';