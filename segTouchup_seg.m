...
% disp('removing disconnected voxels for CSF...') %% OG code below
% siz = sizeOfObject(csf_temp,conn);
% if length(siz)>=4
%     thres = siz(4)+1;
%     csf_temp = bwareaopen(csf_temp,thres,conn);
% end
siz = sizeOfObject(csf_temp,6); %% AJ code
 if length(siz)>=4
     thres = siz(4)+1;
     csf_temp = bwareaopen(csf_temp,thres,6);%% AJ code
 end

% disp('removing disconnected voxels for bone...')
% siz = sizeOfObject(bone_temp,conn);
siz = sizeOfObject(bone_temp,6);%% AJ code
if length(siz)>=2
    ...
    bone_temp = bwareaopen(bone_temp,thres,6);%% AJ code
end

% disp('removing disconnected voxels for skin...')
siz = sizeOfObject(skin_temp,6);%% AJ code changed con for 6
if length(siz)>=2
    thres = siz(2)+1;
    skin_temp = bwareaopen(skin_temp,thres,6);
end
